...
const EMPTY_NAVIGATION_FORM: NavigationForm = {
  label: "",
  url: "",
  placement: "both",
...
function AdminNavigationPage() {
  ...
  const [settings, setSettings] = useState<SiteChromeSettings>(
    DEFAULT_SITE_CHROME_SETTINGS,
  );

  const [items, setItems] = useState<NavigationItem[]>([]);

  const [form, setForm] = useState<NavigationForm>(EMPTY_NAVIGATION_FORM);
      ...
      Array.from(
        new Set(
          items
            .filter(
              (item) =>
                item.placement === "footer" || item.placement === "both",
            )
            .map((item) => item.group_label.trim() || "Navigáció"),
        ),
      ),
    [items],
  );
  ...
  async function loadContent() {
    ...
    if (settingsData) {
      setSettings({
        ...DEFAULT_SITE_CHROME_SETTINGS,
        ...(settingsData as Partial<SiteChromeSettings>),
      });
    }

    const loadedItems = (navigationData ?? []) as NavigationItem[];
  ...
  function updateSettings<K extends keyof SiteChromeSettings>(
    field: K,
  ...
  function updateForm<K extends keyof NavigationForm>(
    field: K,
  ...
  function getNextSortOrder(currentItems: NavigationItem[]) {
  ...
  async function saveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    const headerCtaText = settings.header_cta_text.trim();
    const headerCtaUrl = settings.header_cta_url.trim();
    const announcementText = settings.announcement_text.trim();
    const announcementUrl = settings.announcement_url.trim();

    if (settings.header_cta_visible && (!headerCtaText || !headerCtaUrl)) {
      setErrorMessage(
    ...
    if (settings.announcement_visible && !announcementText) {
      setErrorMessage("Látható értesítési sáv esetén a szöveg kötelező.");
      return;
    }
    ...
    try {
      ...
      setSettings(payload);
      setSuccessMessage("A fejléc és lábléc beállításai elmentve.");
    } catch (error: unknown) {
      setErrorMessage(
        error instanceof Error
          ? error.message
  ...
  function startEditing(item: NavigationItem) {
    ...
    document.getElementById("navigation-editor")?.scrollIntoView({
      behavior: "smooth",
      block: "start",
    });
  }

  async function saveNavigationItem(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();

    const label = form.label.trim();
    const url = form.url.trim();
    const groupLabel = form.group_label.trim() || "Navigáció";

    if (!label) {
      setErrorMessage("A menüpont neve kötelező.");
      return;
    }

    if (!url) {
      setErrorMessage("A célhivatkozás kötelező.");
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        setSuccessMessage("A menüpont sikeresen frissítve.");
      } else {
        const { error } = await supabase
          .from("site_navigation_items")
          .insert(payload);
        ...
        setSuccessMessage("Az új menüpont létrehozva.");
      }
  ...
  async function toggleVisibility(item: NavigationItem) {
    ...
    try {
      ...
      setSuccessMessage(
        item.is_visible ? "A menüpont elrejtve." : "A menüpont láthatóvá téve.",
      );
    } catch (error: unknown) {
      setErrorMessage(
        error instanceof Error
  ...
  async function moveItem(item: NavigationItem, direction: "up" | "down") {
    ...
    try {
      ...
      await loadContent();
      setSuccessMessage("A menüpont sorrendje módosítva.");
    } catch (error: unknown) {
      setErrorMessage(
        error instanceof Error
          ? error.message
  ...
  async function deleteItem(item: NavigationItem) {
    ...
    try {
      ...
      setSuccessMessage("A menüpont törölve.");
    } catch (error: unknown) {
      setErrorMessage(
        error instanceof Error
          ? error.message
    ...
    <main className="min-h-screen bg-muted/30 px-4 py-10 sm:px-6">
      <div className="mx-auto max-w-7xl">
        <header className="mb-8">
          ...
          <h1 className="mt-4 text-3xl font-bold">Fejléc és lábléc</h1>

          <p className="mt-2 max-w-3xl text-muted-foreground">
            Menüpontok, felső értesítési sáv, fejléc-CTA és láblécmegjelenés
            kezelése.
          </p>
        </header>
        ...
        >
          <div className="mb-6">
            <h2 className="text-xl font-bold">Globális megjelenés</h2>

            <p className="mt-1 text-sm text-muted-foreground">
              A logó, kapcsolati adatok és közösségi linkek továbbra is a
              Weboldal adatai menüpontban kezelhetők.
            </p>
          </div>

          <div className="grid gap-6 xl:grid-cols-2">
            <section className="rounded-2xl border p-5">
              <h3 className="font-bold">Felső értesítési sáv</h3>
              ...
              <div className="mt-5 space-y-4">
                <TextField
                  ...
                  onChange={(value) =>
                    updateSettings("announcement_text", value)
                  }
                ...
                <TextField
                  ...
                  onChange={(value) =>
                    updateSettings("announcement_url", value)
                  }
                ...
                <CheckboxField
                  ...
                  onChange={(checked) =>
                    updateSettings("announcement_visible", checked)
                  }
                />
              </div>
            </section>
            ...
            <section className="rounded-2xl border p-5">
              <h3 className="font-bold">Fejléc beállításai</h3>
              ...
              <div className="mt-5 space-y-4">
                <TextField
                  ...
                  onChange={(value) => updateSettings("header_cta_text", value)}
                />
                ...
                <TextField
                  ...
                  onChange={(value) => updateSettings("header_cta_url", value)}
                />

                <CheckboxField
                  label="CTA-gomb látható"
                  description="Asztali és mobil fejlécben is megjelenik."
                  checked={settings.header_cta_visible}
                  onChange={(checked) =>
                    updateSettings("header_cta_visible", checked)
                  }
                />
                ...
                <CheckboxField
                  ...
                  onChange={(checked) =>
                    updateSettings("header_sticky", checked)
                  }
                />
              </div>
            </section>
          ...
          <section className="mt-6 rounded-2xl border p-5">
            <h3 className="font-bold">Lábléc beállításai</h3>

            <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <CheckboxField
                label="Navigáció"
                description="Csoportosított lábléclinkek megjelenítése."
                checked={settings.footer_show_navigation}
                onChange={(checked) =>
                  updateSettings("footer_show_navigation", checked)
                }
              />
              ...
              <CheckboxField
                ...
                onChange={(checked) =>
                  updateSettings("footer_show_contact", checked)
                }
              />
              ...
              <CheckboxField
                ...
                onChange={(checked) =>
                  updateSettings("footer_show_social", checked)
                }
              />
              ...
              <CheckboxField
                ...
                checked={settings.footer_show_back_to_top}
                onChange={(checked) =>
                  updateSettings("footer_show_back_to_top", checked)
                }
              />
            </div>
          </section>
          ...
          >
            {savingSettings ? "Mentés..." : "Megjelenési beállítások mentése"}
          </button>
        </form>
        ...
        <div className="grid gap-8 xl:grid-cols-[420px_1fr]">
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <div>
                <h2 className="text-xl font-bold">
                  {editingId ? "Menüpont szerkesztése" : "Új menüpont"}
                </h2>
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) =>
                  updateForm(
                    "placement",
                    event.target.value as NavigationPlacement,
                  )
                }
                className="w-full rounded-xl border bg-background px-4 py-3 outline-none focus:ring-2 focus:ring-brand"
              >
                <option value="both">Fejléc és lábléc</option>
                <option value="header">Csak fejléc</option>
                <option value="footer">Csak lábléc</option>
              </select>
            </div>
            ...
            <TextField
              ...
              onChange={(value) => updateForm("group_label", value)}
            ...
            <TextField
              ...
              onChange={(value) => updateForm("sort_order", Number(value))}
            />
            ...
            <CheckboxField
              ...
              onChange={(checked) => updateForm("is_visible", checked)}
            />
            ...
            <CheckboxField
              ...
              onChange={(checked) => updateForm("open_in_new_tab", checked)}
            />
          ...
          <section className="rounded-2xl border bg-background shadow-sm">
            <div className="border-b p-5">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold">Menüpontok</h2>

                  <p className="mt-1 text-sm text-muted-foreground">
                    {items.length} elem · {footerGroups.length} lábléccsoport
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="divide-y">
                {items.map((item, index) => {
                  ...
                  return (
                    <article
                      key={item.id}
                      className={`p-5 ${item.is_visible ? "" : "opacity-55"}`}
                    >
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            >
                              {item.is_visible ? "Látható" : "Elrejtett"}
                            </span>
                          ...
                          {(item.placement === "footer" ||
                            ...
                            <p className="mt-2 text-xs text-muted-foreground">
                              Lábléccsoport: {item.group_label || "Navigáció"}
                            </p>
                          )}
...
function placementLabel(placement: NavigationPlacement) {
  switch (placement) {
    case "header":
      return "Csak fejléc";
    case "footer":
